...
def multicore_pattern_set_geter(filename, startP, endP):
...
def get_pattern_set():
    ...
    pp_path = params['extract_pp_dir']
    files = os.listdir(pp_path)
    print('get pattern set.\nprocess...')
    for name in tqdm(files):
        filename = os.path.join(pp_path, name)
        ...
        for i in range(workers):
            ...
            results.append(pool.apply_async(
                multicore_pattern_set_geter, (filename, chunk_start, chunk_end)
            ))
        pool.close()
        pool.join()
    ...
    print('{} patterns left.'.format(sum([global_pa_set[k] for k in global_pa_set if valid_tag(k, global_pa_set[k], v_pp_count)])))
    return {k for k in global_pa_set if valid_tag(k, global_pa_set[k], v_pp_count)}


def judge_content_word(p):
    if p in {'NN', 'NNS'} or p.startswith('VB') or p.startswith('JJ') or p.startswith('RB'):
        return True
    return False
...
if __name__ == '__main__':
    params = {
        # 'cause_vec_path': os.path.join(project_source_path, 'boostvec/en/tutorial/fl_max_cause_11.txt'),
        # 'effect_vec_path': os.path.join(project_source_path, 'boostvec/en/tutorial/fl_max_effect_11.txt'),
        'threshold': {
            'wp': 1,  # 词对出现的次数
            'pa': 1,  # 词对被动词连接的次数
            'v_pp_count': [500, 1500000]
        },
        'num_threads': 15,
        'extract_pp_dir': os.path.join(project_source_path, 'boostvec/en/ppfrag/'),  # 抽取出的短语对的文件
        'pa_set_output_path': os.path.join(project_source_path, 'boostvec/en/dual_pattern_set.txt'),
        'counter_output_path': os.path.join(project_source_path, 'boostvec/en/en_wp_dual_counter.txt'),  # counter保存路径
    }
    if len(sys.argv) > 1:
