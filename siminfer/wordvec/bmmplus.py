...
def load_weak_phrase_pairs(phrase_pairs_path, selected_patterns, max_sent_len, min_count, diff, offset=0.2):
    items = []
    ...
    for line in fin:
        try:
            ...
            if len(arg1_words) != len(arg1_pos) or len(arg2_words) != len(arg2_pos):
                continue
            filtered_arg1 = filter_arg(arg1_words, arg1_pos)
            filtered_arg1 = filtered_arg1[-max_sent_len:]
            filtered_arg2 = filter_arg(arg2_words, arg2_pos)
            filtered_arg2 = filtered_arg2[:max_sent_len]
            if len(filtered_arg1) > max_sent_len and len(filtered_arg2) > max_sent_len:
                continue
            if len(filtered_arg1) and len(filtered_arg2):
                if pattern in selected_patterns:
                    items.append((filtered_arg1, filtered_arg2, pattern))
                rev_pattern = '{}_rev'.format(pattern)
                if rev_pattern in selected_patterns:
                    items.append((filtered_arg2, filtered_arg1, rev_pattern))
        except Exception as e:
            pass
    left_counter, right_counter = WordCounter(), WordCounter()
    for arg1, arg2, pattern in items:
        left_counter.add(arg1)
        right_counter.add(arg2)
    # _left_dict = {w: left_counter[w] for w in left_counter if left_counter[w] >= min_count}
    # _right_dict = {w: right_counter[w] for w in right_counter if right_counter[w] >= min_count}
    _left_set = {k for k in left_counter if left_counter[k] >= min_count}
    _right_set = {k for k in right_counter if right_counter[k] >= min_count}
    response = []
    for arg1, arg2, pattern in items:
        _left = [w for w in arg1 if w in _left_set]
        _right = [w for w in arg2 if w in _right_set]
        if _left and _right:
            p = selected_patterns[pattern]-offset if diff else selected_patterns[pattern]
            response.append((_left, _right, p, 1.0))
    return response, _left_set, _right_set
...
def load_sim_resource(archive, vocab, topk):
    final = dict()
...
class Trainer(object):
    ...
    def calculate_loss(self, left_embed, right_embed, left_mask, right_mask, diff=0.8):
        ...
        # pos_ce = -tf.log(max_prob) * self.targets*(2.*self.pattern_prob-diff)
        pos_ce = -tf.log(max_prob) * self.targets*self.pattern_prob
        _neg_ce = tf.reduce_sum(-tf.log(1.0 - _probs) * mask_matrix, axis=[1, 2]) * (1.0 - self.targets)
        # _total = tf.reduce_sum(left_mask, axis=1)*tf.reduce_sum(right_mask, axis=1)
        # neg_ce = _neg_ce/_total
        neg_ce = _neg_ce
        # # [bs,]
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'resource_path': os.path.join(path, 'en/wp_sim_resources.txt'),  # 单词的近义词及其相似度,使用wordvec计算
        # 词向量保存路径
        'cause_path': os.path.join(path, 'en/embed/bmmplus_cause_v1'),
        'effect_path': os.path.join(path, 'en/embed/bmmplus_effect_v1'),
    ...
    print('\n\n\nStrategy: weak_prob, context_weights=divide_sum, \n')
    for k in parameters:
    ...
    #     pp_dir=parameters['weak_causal_pp_path'], min_count=parameters['weak_w_count'],
    #     workers=parameters['num_threads']
    # )
    weak_pp, weak_left_set, weak_right_set = load_weak_phrase_pairs(
        parameters['weak_causal_pp_path'], pa_prob_dict, parameters['max_len'], parameters['weak_w_count'],
        diff=parameters['weak_p_diff'], offset=parameters['weak_p_offset']
    )
    print('{} weak pp are used for training...'.format(len(weak_pp)))
    print('weak pp vocab: left {}, right {}.'.format(len(weak_left_set), len(weak_right_set)))
    strong_pp, strong_left_set, strong_right_set = load_strong_pp(
        parameters['strong_pp_path'], parameters['strong_w_count']
    )
    print('{} strong pp are used for training...'.format(len(strong_pp)))
    print('strong pp vocab: left {}, right {}.'.format(len(strong_left_set), len(strong_right_set)))
    ...
    resources = load_sim_resource(parameters['resource_path'], left_v|right_v, max_context_len)
    # resources = load_sim_resource(parameters['resource_path'], strong_left_set|strong_right_set, max_context_len)
    ...
    tensor_train_samples = complete_by_multi_core(strong_pp+weak_pp, max_len)
    print('number of pp is {}.'.format(len(tensor_train_samples)))
    # del strong_pp, weak_pp
    annotate_test_set, sharp_wp_set, baseline = load_test_data(
        parameters['annotated_test_path'], parameters['en_wp_testset'], left_indices, right_indices
    )
