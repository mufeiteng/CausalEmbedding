...
def load_sim_resource(archive, vocab, topk, ratio=0.0):
    final = dict()
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        'resource_path': os.path.join(path, 'en/wp_sim_combine.txt'),  # 单词的近义词及其相似度,使用wordnet+wordvec计算
        # 词向量保存路径
        'cause_path': os.path.join(path, 'en/embed/bmmplus_cause_v2'),
        'effect_path': os.path.join(path, 'en/embed/bmmplus_effect_v2'),
    ...
    annotate_test_set, sharp_wp_set, baseline = load_test_data(parameters['annotated_test_path'], parameters['en_wp_testset'], left_indices,
                                                               right_indices)
